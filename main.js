...
function initializeBackgroundVideo() {
    ...
    const playBackground = () => bgVideo.play().catch(() => {
        // The first decoded frame remains visible if a browser blocks playback.
    });

    if (bgVideo.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA) {
        playBackground();
    } else {
        bgVideo.addEventListener('canplay', playBackground, { once: true });
    }
}

function initializeBackgroundParallax() {
    if (!bgVideo || window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    ...
    const updateBackgroundPosition = () => {
        const offsetX = (pointerX - 0.5) * maxOffset * -2;
        const offsetY = (pointerY - 0.5) * maxOffset * -2;
        bgVideo.style.transform = `translate(${offsetX}px, ${offsetY}px) scale(1.06)`;
        frameId = null;
    };
    ...
    window.addEventListener('pointermove', (event) => {
        ...
        if (frameId === null) {
            frameId = window.requestAnimationFrame(updateBackgroundPosition);
        }
    });
...
function initializeCinematicIntro() {
    ...
    const startIntro = () => {
        ...
        introVideo.play().catch(revealPortfolio);
    };
    ...
    introVideo.addEventListener('error', revealPortfolio, { once: true });
}
